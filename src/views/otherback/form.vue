<template>
  <div class="container">
    ...
    <customer ref="customer" :source="sources.customerList" @choose="pickCustomer" @cancel="cancelPicker" />
  </div>
</template>
<script>
import { savebucket } from '@/api/so'
import { newGuid, floatAdd, floatSub } from '@/utils'
import { getInventory, getBucketType, getBucketStatus, getCustomer } from '@/api/base'
...
export default {
  name: `other_back`,
  components: { bucketstatus, buckettype, customer },
  data() {
    this.confirm = 0
    this.BUSTYPE = 4
    return {
  ...
  methods: {
    ...
    pickCustomer({ cCusCode, cCusName }) {
      this.headForm.cCustCode = cCusCode
      this.headForm.cCusName = cCusName
    },
    cancelPicker() {
      this.setFocus()
    }
  ...
  mounted() {
    ...
    setTimeout(() => {
      getCustomer({ FType: this.BUSTYPE })
        .then(({ Data }) => {
          this.sources.customerList = Data
        })
        .catch(err => {})
    }, 150)
